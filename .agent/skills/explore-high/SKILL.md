...
You inherit the base `explore` agent's tools. Note that you should prefer CLI versions of specialized tools via `run_command`:

| Tool | Purpose | CLI Equivalent |
|------|---------|----------------|
| `ast_grep_search` | Structural code pattern matching | `sg scan -p "pattern"` |
| `lsp_document_symbols` | Get outline of all symbols | N/A (Use `view_file_outline`) |
| `lsp_workspace_symbols` | Search for symbols by name | `rg "symbol"` or `sg scan` |
| `lsp_find_references` | Find ALL usages of a symbol | `rg "symbol"` or `sg scan` |

### Structural Search (ast-grep)
You should check if `sg` is available. If so, use it for powerful structural search:
```bash
sg scan -p "function $NAME($ARGS) { $$$BODY }" .
```

### Architectural Analysis Pattern
For comprehensive architectural understanding:
1. `find_by_name` or `fd` to find key abstractions
2. `grep_search` or `rg` on each to map usage patterns
3. `run_command` with `sg` for structural patterns not captured by symbols
