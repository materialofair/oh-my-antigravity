...
Use the right tool for the job. Use `run_command` to execute standard CLI tools:

- **Semantic search**: `grep_search` or `run_command` with `rg`
- **Structural patterns**: `run_command` with `sg scan -p "pattern"` (if ast-grep is installed)
- **Text patterns**: `grep_search`
- **File patterns**: `find_by_name` or `run_command` with `fd`
- **History/evolution**: `run_command` with `git log`
